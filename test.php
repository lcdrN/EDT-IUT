...
$html = "<table border=10px>";
$html= $html . "<tr> <th></th>";
for($z=0;$z<7;$z++){
	$html = $html . "<th>". $heure[$z] ."</th>";
}
...
$html = $html . "<tr><th>".$jour[$compteur++]."</th>";
...
while($nb < 64){
	...
	if($date5->format('d') == $tab3[0]->getDateDeb()->format('d')){
		...
		if(isset($tab3)){
			if(count($tab3) == 1){
				$html = $html . "<td>". $tab3[0]->afficher() ."</td>";
			}
			else{
				$html = $html . "<td><table width=100% height=100%>";
					for($n=0;$n<count($tab3);$n++){
						$html = $html . "<tr style=border-bottom:1pt solid black><td>". $tab3[$n]->afficher() ."</td></tr>";
					}
				$html = $html . "</table></td>";
				}
		}
	...
	$html = $html . "</tr> <tr> <th>".$jour[$compteur++]."</th>";
	...
	$nb_cour_heure = count($tab3);
		...
		$html = $html . "<td><table width=100% height=100% >";
		for($n=0;$n<count($tab3);$n++){
			$html = $html . "<tr><td width=100% height=100%>". $tab3[$n]->afficher() ."</td></tr>";
		}
		$html = $html . "</table></td>";
		}
	}
